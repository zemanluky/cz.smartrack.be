...
import {gatewayDevice, shelfPositionsDevice} from "./device";
...
export const notification = pgTable('notification', {
    ...
    organization_id: integer().references(() => organization.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
...
export const notificationLowBattery = pgTable('notification_low_battery', {
    id: integer().primaryKey().references(() => notification.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
    shelf_positions_device_id: integer().notNull().references(() => shelfPositionsDevice.id, ),
...
export const notificationLowBatteryRelations = relations(notificationLowBattery, ({ one }) => ({
    ...
    shelf_positions_device: one(shelfPositionsDevice, { fields: [notificationLowBattery.shelf_positions_device_id], references: [shelfPositionsDevice.id] }),
}));